...
if __name__ == '__main__':
    try:
        print("🔍 Fetching AWS Health Dashboard events (issue + scheduledChange + notification)...")
        events = get_health_events()
        findings = collect_health_issues(events)

        if findings:
            write_to_csv(findings, OUTPUT_FILE)
            print(f"✅ Exported {len(findings)} health events to {OUTPUT_FILE}")

            if AZURE_PRIMARY_KEY and AZURE_WORKSPACE_ID:
                post_to_log_analytics(AZURE_WORKSPACE_ID, AZURE_PRIMARY_KEY, AZURE_LOG_TYPE, findings)
            else:
